...
lastKOMUT = KOMUT
Dim TRCMD1 As Byte 'transponder'a gonderilecek byte deger1 (hesabi icin interruptta komut alan bolume bak)
Dim TRCMD2 As Byte 'transponder'a gonderilecek byte deger2
TRCMD1 = 0
TRCMD2 = 0
        ...
        If D_A = 1 Then 
            ...
            If R_W = 0 Then   
                If last_REQ = 1 Then 
                    GO_DONE = 1
                    dataout  = BATARYA[0]
                    dataout2 = BATARYA[1] ' dikkat gonderdigim bitlerin 7.,b \ bitinde sorun var.. 128 den kucuk deger yollamali
                Else If last_REQ = 2 Then
                    dataout  = TRCMD1
                    dataout2 = TRCMD2
                EndIf
            EndIf
...
Cikis:
    EndIf
    If ADIF = 1 Then GoSub ADCONready
    If INT0IF = 1 Then GoSub RB0int
    GIE = 1
    Context Restore
    ...
    If DTMF = 12 Then                 '#geldi, bitiriyoruz
        ...
        If DTMFcount = KOMUTlen Then  'dogru sayida rakam var
           ...
           If KOMUT[5] = (CSUM & $0F) Then  'Checksum Dogru 
                ...
                TRCMD1 = lastKOMUT[1] + (lastKOMUT[2] * 16)
                TRCMD2 = lastKOMUT[3] + (lastKOMUT[4] * 16)
           Else
                last_ERR = err_CSUM
           EndIf
        EndIf
